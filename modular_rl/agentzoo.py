...
def make_mlps(ob_space, ac_space, cfg):
    ...
    vfnet = Sequential()
    for (i, layeroutsize) in enumerate(hid_sizes):
        inshp = dict(input_shape=(ob_space.shape[0] + 1,)) if i == 0 else {
...
def make_cnn(ob_space, ac_space, cfg):
    assert isinstance(ob_space, Box)
    print type(ac_space)
    hid_sizes = cfg["hid_sizes"]
    if isinstance(ac_space, Box):
    ...
    net = Sequential()
    net.add(Convolution2D(16, 4, 4, activation='relu',
        dim_ordering='tf',
        input_shape=(ob_space.shape)))
    net.add(Convolution2D(16, 4, 4, activation='relu', dim_ordering='tf'))
    net.add(Flatten())
    ...
    # TODO: should this also be a convnet?
    vfnet = Sequential()
    vfnet.add(Convolution2D(16, 4, 4, activation='relu',
        dim_ordering='tf',
        input_shape=(ob_space.shape)))
    vfnet.add(Convolution2D(16, 4, 4, activation='relu', dim_ordering='tf'))
    vfnet.add(Flatten())
    vfnet.add(Dense(20, activation='relu'))
    vfnet.add(Dense(1))
    baseline = NnVf(vfnet, cfg["timestep_limit"], dict(mixfrac=0.1))
